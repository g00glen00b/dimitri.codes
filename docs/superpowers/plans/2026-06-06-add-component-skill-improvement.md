# add-component Skill Improvement Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Rewrite `.claude/skills/add-component/SKILL.md` to be an accurate, comprehensive briefing document for agents creating Astro components.

**Architecture:** Single file rewrite — no scripts. The skill is agent-invoked as a reference document, so correctness and completeness are the only requirements. The current file has a critical mismatch (scoped `<style>` template) and lacks design token and neo-brutalism pattern guidance.
...
```bash
cat .claude/skills/add-component/SKILL.md
```

- [ ] **Step 2: Write the new file**

Replace the entire contents of `.claude/skills/add-component/SKILL.md` with:
...
Confirm:
- Frontmatter description no longer mentions "scoped styles"
- Template has no `<style>` block
- `export interface Props` (not `interface Props`)
- Design tokens table present with use-case column
- Shadows table present
- Three neo-brutalism pattern sections present (offset shadow, hover press-in, borders)

- [ ] **Step 4: Commit**

```bash
git add .claude/skills/add-component/SKILL.md
git commit -m "feat(skill): rewrite add-component with Tailwind conventions and neo-brutalism patterns"
```
