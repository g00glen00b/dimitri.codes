...
One of the Spring Boot starters that make this motto come true is **Spring Boot Actuator**.
This is because Spring Boot Actuator provides several tools (actuators) to make your job easier to keep your applications properly running in production.
You could say it's your swiss army knife to production!
...
By default, Spring already provides several health indicators, such as one to check your database connection, your disk space, ... .
...
You can change this by changing the following property:
...
Be aware though, some health indicators might expose sensitive information (such as the database vendor you're using).
...
The nice thing about these health groups is that you can configure their HTTP mapping separately, their authorization access requirements, ... .
For example:
...
The liveness and readiness state of an application are **Kubernetes concepts**, which mean the followoing:
...
This can be useful to add certain things to the liveness/readiness probe such as the builtin `DiskSpaceHealthIndicator`, the `DataSourceHealthIndicator` and so on.
